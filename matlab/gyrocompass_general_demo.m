...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2) PICK A POINT ON THE EARTH, INSTALL A CYCLIC TRACK HERE 
...
% we use the radius as a normal vector to generate a tangential plane
% arbitrarily choose two unit vectors to span the plane

% we construct the first unit vector on the tangent plane by arbitrarily
% assiging x- and y- coordinates, then pin down the z-coordinate by setting
% the dot product of this vector and the normal vector to be zero, so taht
% this vector is perpendicular to the normal vector. Then normalize it.
x0 = 50; y0 = 50;
z0 = gyropos(3) + (gyropos(1)*(gyropos(1)-x0) + ...
    gyropos(2)*(gyropos(2)-y0))/gyropos(3);
unitvec1 = [x0-gyropos(1), y0-gyropos(2), z0-gyropos(3)]./ ...
    norm([x0-gyropos(1), y0-gyropos(2), z0-gyropos(3)]);

% the other unit vector should be generated by cross product so that it is
% orthogonal to both the radius and univec1
...
v = VideoWriter('gyrocompass_autoview_test');
open(v);
