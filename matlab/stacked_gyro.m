...
Rearth = 100;
% we divide each latitude line into 6 segments
thetas = 0 : 2*pi/6 : 2*pi - 0.001;
% we cut the ball horiontally to get 5 circles
phis = [-pi/3, -pi/6, 0, pi/6, pi/3];
...
for i = 1:5
    ...
    for j = 1:6
        ...
        % r*cos(phi).
        Xearth[5*(i-1)+j,:] = [Rearth*cos(thetas(j)*sin(phis(i)), Rearth*sin(thetas(j)*sin(phis(i)), Rearth*cos(phis(i))];
